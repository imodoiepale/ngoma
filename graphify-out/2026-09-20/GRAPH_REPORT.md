...
## Corpus Check
- 906 files · ~5,005,815 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5045 nodes · 8387 edges · 496 communities (252 shown, 244 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 396 edges (avg confidence: 0.75)
...
## Communities (496 total, 244 thin omitted)
...
### Community 3 - "RunPod"
Cohesion: 0.06
Nodes (46): main(), mapped_ssh(), pick_pod(), print_urls(), Path, run_bootstrap(), scp_base(), ssh_base() (+38 more)
...
Cohesion: 0.13
Nodes (30): audio_duration(), audio_energy(), build(), main(), Any, Path, Song -> treatment -> shot list, with a generation route per shot.  Implements, Per-section loudness, used to decide which stage ranges carry hero shots. (+22 more)

### Community 24 - "whatsapp.py"
Cohesion: 0.21
Nodes (20): _base(), _headers(), key_for(), _load_state(), main(), _post(), preflight(), Any (+12 more)
...
Cohesion: 0.25
Nodes (7): Dataset Workflow V1, EPALLE Studio (Next.js), Shot Acceptance Criteria (identity drift), RunPod Verification Summary, matrix-power-nodes (compiler-generated, MIT), Semantic Retry Guard Against Double Billing, WhatsApp Send Safeguards (8/day cap, 10-min gap, --i-mean-it, idempotency)
...
Cohesion: 0.06
Nodes (59): audio_to_video(), brand_kit(), captions(), claim_check(), composite_batch(), concat(), copy_block(), EditError (+51 more)

### Community 46 - "idea_diagrams.py"
Cohesion: 0.14
Nodes (35): _archify(), archify_home(), build(), cards(), _clip(), deliver(), describe(), dumps() (+27 more)
...
Cohesion: 0.39
Nodes (14): bind_folder(), bind_models(), comfy_python(), copy_workflows(), find_comfy(), install_nodes(), _is_placeholder(), log() (+6 more)
...
Cohesion: 0.12
Nodes (13): extract(), folder_for(), main(), Return (node_type, model_filename) pairs from a UI-format or API-format graph., build(), harvest(), listings(), main() (+5 more)
...
Cohesion: 0.10
Nodes (26): budget_usd(), config(), estimate(), estimate_node(), estimate_nodes(), max_items(), Any, What a step would cost before it runs. Every figure says whether it was measured (+18 more)
...
Cohesion: 0.25
Nodes (11): fetch(), cost(), _run(), test_a_budget_of_zero_blocks_spending_even_when_approved(), test_a_stage_whose_inputs_were_never_made_is_blocked_not_faked(), test_an_input_that_is_not_on_disk_is_an_error_not_a_submission(), test_approved_stages_within_budget_run_and_complete(), test_auto_needs_one_engine_run_approval_then_stops_at_the_pick() (+3 more)
...
Cohesion: 0.13
Nodes (24): check_brief(), EngineBrief, Any, What the director engine is told, and what it decides. Plain dataclasses, JSON i, A folder of references. `use: data` feeds generation directly; `inspiration` onl, RefCollection, Role, add_role() (+16 more)

### Community 404 - "plan"
Cohesion: 0.20
Nodes (15): Scene, ShotSpec, branch_from_picks(), Builder, DirectorError, expand_angles(), _first(), plan() (+7 more)

### Community 405 - "presets.py"
Cohesion: 0.19
Nodes (17): set_preset(), cameras(), compose_motion_prompt(), _cycle(), load_presets(), negatives(), node_params(), preset() (+9 more)
...
Cohesion: 0.11
Nodes (32): Centred Frontal Symmetry with Single Seated Anchor, Composed Stillness / Fashion-Film Restraint, Crushed Pure-Black Surround (detail-free frame edges), Suspended Falling Petals, Film-Poster Grammar (title stack over a still, logline, tagline footer), Floral Head Substitution Motif (bouquet replacing the face), COVER ART - 'Flowers Falling' Single Cover, Letterspaced Small-Caps Logline ('a man attends the wedding-funeral...') (+24 more)
...
Cohesion: 0.10
Nodes (19): _authored(), _edit_world(), _manifest_of(), _persona(), The runner: dry runs write manifests and submit nothing; modes gate spending; pi, A finished clip on every motion step and the pick made, so the edit stage has in, An authored (stage-less) workflow for the scratch client, saved so run_stage can, _result() (+11 more)
...
Cohesion: 0.09
Nodes (34): Namespace, cmd_describe(), cmd_plan(), cmd_ports_check(), cmd_possibilities(), cmd_run(), cmd_say(), main() (+26 more)
...
Cohesion: 0.25
Nodes (8): apply_answers(), from_brief(), known_collections(), A swap needs the images to change and the face to put in; a wardrobe step needs, A plain-language brief becomes a workflow. `plan` is a `parse_brief` result to r, Reference collections on disk for a client: folders under brands/<client>/refere, Answers to the plan's questions change the plan in place and clear the question., _split_shared_inputs()
...
Cohesion: 0.20
Nodes (15): _accepts(), _cycle(), _depths(), _fans_out(), is_each(), iterated_port(), layout_lanes(), load_catalog() (+7 more)

### Community 479 - "session/route.js"
Cohesion: 0.18
Nodes (11): _coerce(), _find_known(), _literal_steps(), _makes_media(), _number(), parse_brief(), Known collection names appearing in the text, longest match first, in text order, `carousel@each slides=10` (what the canvas sends) is steps and params, not prose (+3 more)

### Community 484 - "test_idea_diagrams.py"
Cohesion: 0.29
Nodes (9): ir_path(), load_ir(), Path, Idea workflow diagrams: one Archify IR per business idea, generated and never ha, test_every_template_step_is_drawn_with_its_gates(), test_html_is_delivered_next_to_its_source(), test_ir_regenerates_byte_identically(), test_ir_shape_and_references() (+1 more)
