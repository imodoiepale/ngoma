...
const VINFO_ROUTE = "/icy_loader/vinfo";
const PREVIEW_ROUTE = "/icy_loader/preview";
...
.icy-videopreview{width:100%;background:#0d1117;border-radius:8px;overflow:hidden}
.icy-videopreview video{width:100%;display:block}
.icy-video-box{position:relative}
.icy-video-loading{position:absolute;inset:0;display:flex;flex-direction:column;gap:8px;align-items:center;
  justify-content:center;background:rgba(8,12,18,.72);color:#9fd4ff;font-size:11px}
.icy-video-loading[hidden]{display:none}
.icy-video-loadbar{width:60%;height:4px;border-radius:2px;background:#22303f;overflow:hidden}
.icy-video-loadbar div{height:100%;width:0%;background:#7cc9ff;border-radius:2px;transition:width .25s}
.icy-video-stats{display:flex;flex-direction:column;gap:2px;padding:5px 8px;font-size:11px;line-height:1.4;
  background:#0d1117;border-top:1px solid #27303d}
.icy-video-stats:empty{display:none}
.icy-stat-row{display:flex;align-items:baseline;gap:6px;min-width:0}
.icy-stat-tag{flex:none;padding:0 5px;border-radius:5px;font-size:9px;font-weight:600;letter-spacing:.4px;
  text-transform:uppercase}
.icy-stat-src{background:#22303f;color:#9fd4ff}
.icy-stat-load{background:#1d2b22;color:#9fe6a8}
.icy-stat-val{flex:1;min-width:0;color:#cfd6e0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
`;
...
class GalleryViewer {
  constructor(onSelect, currentFolder, multi = false, initialSel = null, mediaMode = "image") {
    this.onSelect = onSelect;
    this.multi = multi;
    this.mediaMode = mediaMode; // "image" | "media" | "video"
  ...
  noun() {
    if (this.mediaMode === "video") return "video";
    return this.multi ? "media" : "image";
  }

  mediaQuery() {
    if (this.mediaMode === "video") return "&media=video";
    if (this.mediaMode === "media" || this.multi) return "&media=1";
    return "";
  }
  ...
  async refresh() {
    ...
    try {
      const resp = await api.fetchApi(`${LIST_ROUTE}?folder=${this.activeFolder}${this.mediaQuery()}`);
  ...
  _buildCard(item) {
    ...
    card.className = "icy-card" + (this.mediaMode !== "video" && isVideo(item.name) ? " has-video" : "");
  ...
  img.onload = () => {
    ...
    try {
      if (typeof node.setSizeForImage === "function") {
        ...
        const base = node.computeSize?.([node.size[0], node.size[1]])?.[1] ?? node.size[1];
...
function applyVideoSelection(node, folder, item) {
  setFolderWidget(node, folder);

  const videoW = findWidget(node, "video");
  if (videoW) {
    // make the combo accept the chosen value (esp. for output-folder picks)
    videoW.options = videoW.options || {};
    if (Array.isArray(videoW.options.values) && !videoW.options.values.includes(item.path)) {
      videoW.options.values.unshift(item.path);
    }
    videoW.value = item.path;
    if (typeof videoW.callback === "function") { try { videoW.callback(item.path); } catch {} }
  }
...
function fitNodeHeight(node) {
  try {
    // VHS fitHeight: exact size, not grow-only; computeSize takes the output array
...
function videoViewParams(node) {
  const folder = findWidget(node, "folder")?.value || "input";
  const val = (findWidget(node, "video")?.value || "").trim();
  if (!val) return null;
  const parts = val.split("/");
  const params = new URLSearchParams({ filename: parts.pop(), type: folder });
  const subfolder = parts.join("/");
  if (subfolder) params.set("subfolder", subfolder);
  return params;
}

async function updateVideoStats(node) {
  const el = node._icyVideoStats;
  if (!el) return;
  const params = videoViewParams(node);
  if (!params) {
    node._icyVInfo = null;
    el.replaceChildren();
    return;
  }
  const seq = (node._icyStatsSeq = (node._icyStatsSeq || 0) + 1);
  try {
    const resp = await api.fetchApi(`${VINFO_ROUTE}?${params.toString()}`);
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const v = await resp.json();
    if (seq !== node._icyStatsSeq) return;
    node._icyVInfo = v;
    renderStats(node);
  } catch {
    if (seq === node._icyStatsSeq) {
      node._icyVInfo = null;
      el.replaceChildren();
    }
  }
...
function widgetNum(node, name, fallback) {
  const v = Number(findWidget(node, name)?.value);
  return Number.isFinite(v) ? v : fallback;
}

function videoLoadConfig(node, v) {
  const force_rate = widgetNum(node, "force_rate", 0);
  const cap = widgetNum(node, "frame_load_cap", 0);
  const skip = Math.max(0, widgetNum(node, "skip_first_frames", 0));
  const nth = Math.max(1, widgetNum(node, "select_every_nth", 1));
  const eff = force_rate > 0 ? force_rate : v.fps || 0;
  return { force_rate, cap, skip, nth, eff };
}

function statRow(tag, tagClass, text) {
  const row = document.createElement("div");
  row.className = "icy-stat-row";
  const t = document.createElement("span");
  t.className = `icy-stat-tag ${tagClass}`;
  t.textContent = tag;
  const v = document.createElement("span");
  v.className = "icy-stat-val";
  v.textContent = text;
  row.append(t, v);
  return row;
}
...
function renderStats(node) {
  const el = node._icyVideoStats;
  if (!el) return;
  const v = node._icyVInfo;
  if (!v) {
    el.replaceChildren();
    el.title = "";
    return;
  }
  const line1 = `${v.width}×${v.height} · ${v.fps} fps · ${v.frames} frames · ${v.duration.toFixed(1)}s · ${humanSize(v.size)}`;

  const { force_rate, cap, skip, nth, eff } = videoLoadConfig(node, v);
  let line2;
  if (eff > 0) {
    const resampled = force_rate > 0 && v.fps ? Math.round(v.frames * eff / v.fps) : v.frames;
    const avail = Math.max(0, Math.floor((resampled - skip) / nth));
    const count = cap > 0 ? Math.min(cap, avail) : avail;
    line2 =
      `${cap > 0 ? `${count} of ~${avail}` : `all ~${avail}`} frames` +
      `${nth > 1 ? ` · every ${nth}th` : ""} · from ${(skip / eff).toFixed(1)}s · @ ${Number(eff.toFixed(2))} fps`;
  } else {
    line2 = "unknown source fps · set force_rate to resample";
  }
  el.replaceChildren(statRow("source", "icy-stat-src", line1), statRow("loads", "icy-stat-load", line2));
  el.title = `${line1}\n${line2}`;
}

function updateVideoPreview(node) {
  const video = node._icyVideoEl;
  const inner = node._icyVideoInner;
  const w = node._icyVideoWidget;
  if (!video) return;
  const params = videoViewParams(node);
  if (!params) {
    video.removeAttribute("src");
    video.load();
    // VHS pattern: hide the inner element and collapse the widget
    inner.hidden = true;
    w.aspectRatio = 0;
    fitNodeHeight(node);
  } else {
    inner.hidden = false;
    // re-encoded stream: the preview shows exactly what the node will load
    params.set("force_rate", widgetNum(node, "force_rate", 0));
    params.set("cap", widgetNum(node, "frame_load_cap", 0));
    params.set("skip", widgetNum(node, "skip_first_frames", 0));
    params.set("nth", widgetNum(node, "select_every_nth", 1));
    const url = `${PREVIEW_ROUTE}?${params.toString()}`;
    if (!video.src.endsWith(url)) video.src = url;
  }
  updateVideoStats(node);
}

function buildVideoPreview(node) {
  if (typeof node.addDOMWidget !== "function") return;
  injectCss();

  const wrap = document.createElement("div");
  wrap.className = "icy-videopreview";

  const inner = document.createElement("div");
  inner.className = "icy-video-box";
  inner.hidden = true; // nothing selected: widget collapses (VHS pattern)
  wrap.appendChild(inner);
  node._icyVideoInner = inner;
  ...
  node._icyVideoEl = video;

  // loading veil: spinner + label + buffer progress bar while the
  // server-side re-encode streams in
  const veil = document.createElement("div");
  veil.className = "icy-video-loading";
  veil.hidden = true;
  const spin = document.createElement("div");
  spin.className = "icy-spinner";
  const label = document.createElement("span");
  const bar = document.createElement("div");
  bar.className = "icy-video-loadbar";
  const barFill = document.createElement("div");
  bar.appendChild(barFill);
  veil.append(spin, label, bar);
  inner.appendChild(veil);
  const updateBar = () => {
    try {
      const end = video.buffered.length ? video.buffered.end(video.buffered.length - 1) : 0;
      barFill.style.width = video.duration ? `${Math.min(100, (end / video.duration) * 100).toFixed(1)}%` : "0%";
    } catch {}
  };
  video.addEventListener("loadstart", () => {
    label.textContent = "encoding preview…";
    barFill.style.width = "0%";
    veil.hidden = false;
  });
  video.addEventListener("progress", updateBar);
  video.addEventListener("canplay", () => { veil.hidden = true; updateBar(); });
  video.addEventListener("waiting", () => {
    label.textContent = "buffering…";
    veil.hidden = false;
  });
  video.addEventListener("error", () => { veil.hidden = true; });

  const stats = document.createElement("div");
  stats.className = "icy-video-stats";
  inner.appendChild(stats);
  node._icyVideoStats = stats;

  const w = node.addDOMWidget("icy video preview", "icy_videopreview", wrap, {
    serialize: false,
    hideOnZoom: false,
  });
  node._icyVideoWidget = w;
  // VHS pattern: derive the height from node.size[0], never from the width
  // argument — the framework calls computeSize() with no args when laying out
  w.computeSize = (width) => {
    if (!inner.hidden && w.aspectRatio) {
      return [width ?? node.size[0], (node.size[0] - 20) / w.aspectRatio + 46];
    }
    return [width ?? node.size[0], -4];
  };

  video.addEventListener("loadedmetadata", () => {
    w.aspectRatio = video.videoWidth / video.videoHeight || 0;
    fitNodeHeight(node);
  });
  // muted while autoplaying; hover the preview to hear it (VHS-style)
  inner.addEventListener("pointerenter", () => { video.muted = false; });
  inner.addEventListener("pointerleave", () => { video.muted = true; });

  const mediaChanged = () => updateVideoPreview(node);
  const settingsChanged = () => {
    renderStats(node);
    // rebuilding the preview stream re-encodes server-side: debounce spinner spam
    clearTimeout(node._icyPreviewTimer);
    node._icyPreviewTimer = setTimeout(() => updateVideoPreview(node), 200);
  };
  for (const [name, cb] of [
    ["folder", mediaChanged],
    ["video", mediaChanged],
    ["force_rate", settingsChanged],
    ["frame_load_cap", settingsChanged],
    ["skip_first_frames", settingsChanged],
    ["select_every_nth", settingsChanged],
  ]) {
...
async function uploadVideoFile(node, file) {
  toast(`Uploading ${file.name}…`);
  try {
    const info = await uploadMedia(file);
    const path = info.subfolder ? `${info.subfolder}/${info.name}` : info.name;
    setFolderWidget(node, "input"); // uploads always land in the input folder
    const videoW = findWidget(node, "video");
    if (videoW) {
      videoW.options = videoW.options || {};
      if (Array.isArray(videoW.options.values) && !videoW.options.values.includes(path)) {
        videoW.options.values.unshift(path);
      }
      videoW.value = path;
      if (typeof videoW.callback === "function") { try { videoW.callback(path); } catch {} }
    }
    updateVideoPreview(node);
    toast(`Loaded ${info.name}`);
  } catch (err) {
    toast(`Upload failed: ${file.name}`, "error");
    console.warn("Icy Loader video upload failed:", err);
  }
}

function addVideoUpload(node) {
  ...
  input.addEventListener("change", async () => {
    ...
    await uploadVideoFile(node, input.files[0]);
    input.value = "";
  });
  const w = node.addWidget("button", "⬆️ Upload Video", "upload", () => {
...
function nodeUnderEvent(e) {
  try {
    ...
    if (n?.icyMulti || n?.icyVideo) return n;
    return null;
  } catch {
    return null;
  }
...
function wireDrop() {
  ...
  document.addEventListener(
    ...
    (e) => {
      const n = nodeUnderEvent(e);
      setDragNode(null);
      if (!n) return; // only drops landing on the node itself are ours
      e.preventDefault();
      e.stopImmediatePropagation();
      if (n.icyVideo) {
        const file = [...(e.dataTransfer?.files || [])].find((f) => f.type.startsWith("video/") || isVideo(f.name));
        if (file) uploadVideoFile(n, file);
        return;
      }
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    const isVideoNode = nodeData.name === "IcyVideoLoader";
    if (!isMulti && !isVideoNode && nodeData.name !== "IcyImageLoader") return;

    if (isMulti || isVideoNode) {
      ...
      nodeType.prototype.onDrawBackground = function (ctx) {
        ...
        if (this._icyDragOver) {
          ...
          ctx.fillText(isVideoNode ? "❄ drop video to load" : "❄ drop media to add references", w / 2, h / 2);
          ctx.restore();
        }
        return r;
      };
      ...
      try {
        if (isMulti) {
          ...
          this.addWidget("button", "🖼️ Open Gallery", "gallery", () => {
            ...
            const viewer = new GalleryViewer(
              (folder, item) => applyVideoSelection(this, folder, item),
              folderW ? folderW.value : "input",
              false,
              null,
              "video"
          ...
          buildVideoPreview(this);
          this.icyVideo = true;
          this.onDropFile = (file) => {
            if (file.type.startsWith("video/") || isVideo(file.name)) {
              uploadVideoFile(this, file);
              return true;
            }
            return false;
          };
        ...
        // legacy canvas keeps the backend-provided node size: grow to fit
        // the widgets/DOM widgets we just added
        fitNodeHeight(this);
      } catch (e) {
    ...
    if (isVideoNode) {
      ...
      nodeType.prototype.onConfigure = function () {
        ...
        try { updateVideoPreview(this); } catch {}
        return r;
      };
    }
  },
