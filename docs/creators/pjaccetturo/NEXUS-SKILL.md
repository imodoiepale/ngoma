...
Source: `source/nexus-animation-skill-guide.docx`, "PJ's Nexus Animation Skill Guide",
published by PJ Accetturo (@pjaccetturo) as a paste-in LLM system prompt alongside a short
film made with it; downloaded 2026-09-19. Study material; see `docs/LICENSING.md`. His
newsletter (pjace.beehiiv.com) and an X thread cover the image and board steps before this.

What it is: one unified skill for writing production-ready video-generation prompts, fusing
three disciplines in one voice: world and character control, camera/physics/optics
precision, and truthful character performance. Input is the scene in plain language, who is
in it (`@names`), how many scenes or cuts, and what each reference image is for. Output is
one finished prompt per scene: reference blocks, a scene-context paragraph, a tech-spec
line, a numbered CUT list, a sound-design line, nothing else.

How our repo uses it: `.claude/skills/shot-prompt-architect` restates the method in our own
words and fuses it with AiKAMI's five-block structure (`docs/creators/aikami/NOTES.md`). The
skill below is kept as PJ published it so the restatement can be checked against it. Where
the two disagree, our skill wins, in particular:

- No `@name` block may describe a real person; only owned or fictional characters.
- We keep PJ's "no em dash" rule and 4,000-character ceiling as-is.
- "100% matches the reference" is kept only for owned reference sheets.

## Usage notes, from the guide

Give it every time: the scene, who is in it, how many scenes or cuts, references and what
each is for. It defaults to one continuous take per scene unless cuts are asked for.

Feedback shorthand it understands:

| You say | It does |
| --- | --- |
| "Too short" | lengthens shots, more character budget per shot |
| "Looks like a knight / a statue" | strips helmet, visor, stone language from face coverings |
| "Feels fake, robotic, over-acted" | rewrites performance around objective, tactic, visible beat changes |
| "Eyes look dead" | adds saccades, blink behaviour, catchlights tied to state |
| "Background too busy or too plain" | one specific world detail behind the human moment |
| "Fewer scenes than I asked for" | recounts and redelivers |
| "Too floaty / weightless" | enforces gravity, weight transfer, follow-through |
